...
**FeedDetailScreen 左右滑动数据源规则**：

- 详情页左右滑动（Pager）的文章列表由导航层根据“进入详情页时的分组/分类”构建，确保只在当前分组内切换文章。
- “全部”（空字符串）：使用过滤后的全量列表（与首页“全部”展示一致）。
- “我的收藏”：使用收藏列表。
- 其他分类/来源：使用当前分组下的子列表。
- 播放中合并播放器播放列表的逻辑仅在“全部”里生效，避免其他分类/收藏混入跨分组文章。
